...
void ContainmentInterface::setPlasmoid(QObject *plasmoid)
{
    ...
    if (m_plasmoid) {
        //! Plasma 6: the containment graphic item does not expose the applet configuration
        //! any longer, it is reachable through its "plasmoid" (Plasma::Applet) property.
        QObject *applet = m_plasmoid->property("plasmoid").value<QObject *>();

        if (!applet) {
            applet = m_plasmoid;
        }

        m_configuration = qobject_cast<KConfigPropertyMap *>(applet->property("configuration").value<QObject *>());
