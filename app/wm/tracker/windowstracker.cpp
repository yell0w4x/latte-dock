...
void Windows::cleanupFaultyWindows()
{
    for (const auto &key : m_windows.keys()) {
        ...
        if (winfo.wid().isNil() || winfo.geometry() == QRect(0, 0, 0, 0)) {
...
void Windows::updateHints(Latte::View *view)
{
    ...
    for (const auto &winfo : m_windows) {
        ...
        if (!existsFaultyWindow && (winfo.wid().isNil() || winfo.geometry() == QRect(0, 0, 0, 0))) {
    ...
    setActiveWindowMaximized(view, (!maxWinId.isNil() && (maxWinId == activeTouchWinId || maxWinId == activeTouchEdgeWinId)));
